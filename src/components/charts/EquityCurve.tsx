      ...
      <div className="h-[220px]">
        <ResponsiveContainer width="100%" height="100%">
          <AreaChart data={data}>
            ...
            <XAxis dataKey="date" stroke="hsl(0, 0%, 20%)" tick={{ fill: 'hsl(0, 0%, 45%)', fontSize: 10 }} tickLine={false} axisLine={false} />
            <YAxis stroke="hsl(0, 0%, 20%)" tick={{ fill: 'hsl(0, 0%, 45%)', fontSize: 10 }} tickLine={false} axisLine={false} tickFormatter={(v) => `$${v}`} />
            <Tooltip contentStyle={{ backgroundColor: '#161616', border: '1px solid hsl(0, 0%, 16%)', borderRadius: '8px', color: '#eee', fontSize: 11 }}
              formatter={(value: number) => [`$${value.toFixed(2)}`, 'Balance']} />
            <Area type="monotone" dataKey="balance" stroke={isPositive ? '#22C55E' : '#EF4444'} strokeWidth={2} fill="url(#equityGradient)" />
          </AreaChart>
        </ResponsiveContainer>
      </div>
    </motion.div>
