...
export function RecentTrades({ trades }: Props) {
  ...
  return (
    <>
      ...
      <div className="rounded-xl bg-card border border-border p-5 px-6 h-full flex flex-col">
        {/* Title + filter pills */}
        <div className="flex items-center justify-between mb-1">
          <span className="text-[10px] font-semibold uppercase tracking-[0.08em] text-muted-foreground/60">Recent Trades</span>
          {!isEmpty && (
                  ...
                  <div className="flex items-center gap-2 py-2.5">
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2">
                        ...
                        {t.session && (
                          <span className="text-[10px] text-muted-foreground/60">{t.session}</span>
                        )}
                        {t.screenshotUrl && (
                          <button
                            onClick={() => setViewingScreenshot(t.screenshotUrl!)}
                            className="flex items-center gap-0.5 text-muted-foreground/50 hover:text-muted-foreground transition-colors outline-none"
                    ...
                    <span className={cn(
                      ...
                      t.pnl > 0 ? 'text-[#10b981]' : t.pnl < 0 ? 'text-[#f87171]' : 'text-foreground'
                    )}>
                      {t.pnl >= 0 ? '+' : ''}${t.pnl.toFixed(2)}
                    </span>
                  </div>
                  {i < recent.length - 1 && <div className="h-px bg-border" />}
                </div>
              ))}
            </div>
          )}
        ...
        {!isEmpty && (
          <Link to="/journal" className="text-[11px] text-muted-foreground/60 hover:text-muted-foreground self-end mt-3 transition-colors">
            View all →
          </Link>
        )}
      </div>
