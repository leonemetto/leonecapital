    ...
    <div className="rounded-xl bg-card border border-border p-5 px-6 h-full flex flex-col">
      ...
      <div className="flex-1 space-y-0">
        {sessions.map(s => {
          ...
          return (
            <div key={s.session} className="py-2.5 border-b border-border/50 last:border-0">
        ...
        {sessions.length === 0 && (
          <p className="text-xs text-muted-foreground/50 text-center py-8">No session data yet</p>
        )}
      </div>
    </div>
  );
