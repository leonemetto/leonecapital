...
export function HeatMapCalendar({ trades }: Props) {
  ...
  return (
    <TooltipProvider delayDuration={200}>
      <div className="rounded-xl bg-card border border-border p-5 px-6">
        {/* Header */}
        ...
        <div className="grid grid-cols-5 gap-1 mb-1">
          {WEEKDAYS.map(d => (
            <div key={d} className="text-center text-[10px] font-medium text-muted-foreground/50 uppercase tracking-widest py-1">
              {d}
            </div>
          ))}
        </div>
                  ...
                  <Tooltip key={di}>
                    <TooltipTrigger asChild>
                      <div
                        ...
                        style={{
                          backgroundColor: data ? pnlIntensity(data.pnl, maxAbsPnl) : 'var(--ef-bg-sunken)',
                          border: '0.5px solid var(--ef-line)',
                        }}
                      >
                        <span className={cn(
                          'text-[10px] font-medium tabular-nums leading-none',
                          data ? 'text-[var(--ef-ink-2)]' : 'text-[var(--ef-ink-4)]'
                        )}>{format(day, 'd')}</span>
                        {data && (
                          <span className={cn(
                            'text-[9px] font-mono font-bold tabular-nums leading-none self-end',
                            data.pnl > 0 ? 'text-[#10b981]' : data.pnl < 0 ? 'text-[#f87171]' : 'text-[var(--ef-ink-4)]'
                          )}>
